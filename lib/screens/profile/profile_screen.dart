...
class _ProfileScreenState extends State<ProfileScreen> {
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        GestureDetector(
                          onTap: () async {
                            // Fetch the follow data using FollowService
                            final followService = FollowService();
                            final followData = await followService.getFollows();

                            if (followData['status'] == 'success') {
                              // Map the followed_list to a list of Follower objects
                              List<Follower> followedList = List<String>.from(followData['followed_list']).map((username) {
                                return Follower(
                                  name: username,
                                  subtitle: "Đang theo dõi bạn", // Text indicating that this user is following the logged-in user
                                  profileImageUrl: "https://via.placeholder.com/150", // Placeholder or actual profile image URL
                                  isFollowing: true, // Assuming they are following the logged-in user
                                );
                              }).toList();

                              // Navigate to FollowerListScreen with the followed list
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => FollowerListScreen(
                                    title: 'Followers',  // Title for the screen
                                    followers: followedList,  // Pass the followed list here
                                    suggestedUsers: [],  // Empty or optional suggested users
                                  ),
                                ),
                              );
                            } else {
                            ...
                            if (followData['status'] == 'success') {
                              List<Follower> followingList = List<String>.from(followData['following_list']).map((username) {
                                return Follower(
                                  ...
                                  profileImageUrl: "https://via.placeholder.com/150", // Placeholder hoặc URL thật
                                  isFollowing: true,
                                );
                              }).toList();
                  ...
                  Padding(
                    ...
                    child: StaggeredGrid.count(
                      ...
                      children: [
                        _buildPostGridTile('assets/images/Rectangle-5.png'),
                        _buildPostGridTile('assets/images/Rectangle-7.png'),
                        _buildPostGridTile('assets/images/Rectangle-8.png'),
                        _buildPostGridTile('assets/images/Rectangle-1.png'),
                      ],
                    ),
                  ),
                ],
  ...
  // Helper function to build each grid tile
  Widget _buildPostGridTile(String imagePath) {
